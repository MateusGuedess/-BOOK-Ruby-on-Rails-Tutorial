...
class UserTest < ActiveSupport::TestCase
  def setup
    @user = User.new(name: "Example User", email: "user@example.com", password: "foolsh", password_confirmation: "foolsh")
  end
  ...
  test "name should not be upper than 50 characters" do
    @user.name = "#{"a" * 51}"
    assert_not @user.valid?
  end
  ...
  test "email should not be upper than 50 characters" do
